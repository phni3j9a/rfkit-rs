...
Each input sample is scaled to a deliberately boundary-separated scalar
`numpy.linalg.svd(..., compute_uv=False)` maximum. Only the dimensionless
`data.sigma_max` output is numerically tolerant; source arrays,
frequency/reference metadata, sample classes, and `data.is_passive` remain
exact contract fields. The latter is generated by one public pinned scikit-rf
`Network.is_passive(tol=1e-12)` call per sample solely as limited Boolean
evidence away from sigma=1; it is not the Rust API or a broad passivity
compatibility claim.
