...
One sampled maximum power-wave singular-value case is also registered:
`max_singular_value_power_four_port_complex_z0` is an independently seeded
four-frequency, coupled non-reciprocal four-port fixture using NumPy
`default_rng` seed `20260957` and unequal complex positive-real references.
Each exact input sample is the seeded raw S stack multiplied by fixed
binary-exact factors `[1.0, 2.0, 4.5, 5.0]`; input construction never calls
SVD, so the exact source arrays do not depend on a platform LAPACK backend.
Public NumPy `numpy.linalg.svd(..., compute_uv=False)` is called only after
input construction for the tolerant `data.sigma_max` output and the derived
sample classes. Only the dimensionless `data.sigma_max` output is numerically
tolerant; source arrays, frequency/reference metadata, sample classes, and
`data.is_passive` remain exact contract fields. The latter is generated by one
