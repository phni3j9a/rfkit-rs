...
INNER_CONNECT_DIRECT_CASE_SPECS = {
    oracle.INNER_CONNECT_DIRECT_CASE_ID: {
...
TERMINATION_CASE_SPECS = {
    ...
    "power_wave_terminate_port_mixed_open_five_port_complex_z0": {
        ...
        "port": 2,
        "survivors": [0, 1, 3, 4],
        "loads": ["open", 31.0 + 7.0j, "open", -17.0 + 4.0j],
        "seed": 20_260_963,
        "output": "s_terminated",
    },
}
...
class DirectInnerConnectRegistrationAndCheckerTests(unittest.TestCase):
    """Protect the direct complex-reference inner-connect oracle contract."""

    @classmethod
    def setUpClass(cls) -> None:
        cls.np, cls.skrf = oracle._load_dependencies()
        cls.case_id = oracle.INNER_CONNECT_DIRECT_CASE_ID
    ...
    def test_case_is_registered_with_strict_direct_wave_contract(self) -> None:
        spec = INNER_CONNECT_DIRECT_CASE_SPECS[self.case_id]
        self.assertEqual(self.case.path, oracle.INNER_CONNECT_DIRECT_FIXTURE)
        ...
        self.assertEqual(metadata["operation"], spec["operation"])
        self.assertEqual(metadata["input_recipe"], oracle.INNER_CONNECT_DIRECT_INPUT_RECIPE)
        self.assertEqual(metadata["numpy_version"], oracle.EXPECTED_NUMPY_VERSION)
        self.assertEqual(metadata["scikit_rf_version"], oracle.EXPECTED_SCIKIT_RF_VERSION)
        self.assertEqual(metadata["scikit_rf_commit"], oracle.EXPECTED_SCIKIT_RF_COMMIT)
        self.assertEqual(metadata["random_seed"], spec["seed"])
        ...
        self.assertEqual(metadata["wave_definition"], "power")
        self.assertEqual(
            metadata["wave_definitions"],
            {
                "input": "power",
                "inner_connect_raw": "pseudo",
                "output": "power",
                "restoration": (
        ...
        self.assertEqual(metadata["tolerance_policy"]["atol"], 1e-12)
        self.assertIn(
            "only data.s_inner_connected",
            metadata["tolerance_policy"]["comparison"],
        )
        self.assertIn("innerconnect", metadata["tolerance_policy"]["justification"])
        self.assertIn("renormalize", metadata["tolerance_policy"]["justification"])

        self.assertEqual(
            [value["real"] for value in data["z0_ohm"][0]],
            [39.0, 68.0, 44.8, 101.0, 50.6],
        )
        self.assertTrue(
            all(
                self._complex(value).real > 0.0
                and self._complex(value).imag != 0.0
                for row in data["z0_ohm"]
                for value in row
            )
        )
        for frequency in range(spec["frequencies"]):
            selected_a = data["z0_ohm"][frequency][spec["junction_port_a"]]
            selected_b = data["z0_ohm"][frequency][spec["junction_port_b"]]
            self.assertNotEqual(selected_a, selected_b)
            self.assertEqual(
                data["z0_inner_connected_ohm"][frequency],
                [
                    data["z0_ohm"][frequency][0],
                    data["z0_ohm"][frequency][2],
                    data["z0_ohm"][frequency][4],
                ],
            )
        self.assertTrue(
            any(
                data["s"][frequency][1][3] != data["s"][frequency][3][1]
                and data["s"][frequency][1][3] != {"real": 0.0, "imag": 0.0}
                and data["s"][frequency][3][1] != {"real": 0.0, "imag": 0.0}
                for frequency in range(spec["frequencies"])
            )
        )

    def test_builder_calls_innerconnect_then_public_power_restoration(self) -> None:
        with mock.patch.object(
            self.skrf.network,
            "innerconnect",
            wraps=self.skrf.network.innerconnect,
        ) as innerconnect, mock.patch.object(
            self.skrf.Network,
            "renormalize",
            autospec=True,
            side_effect=self.skrf.Network.renormalize,
        ) as renormalize:
            document = self.case.builder(self.np, self.skrf)

        self.assertEqual(innerconnect.call_count, 1)
        self.assertEqual(
            innerconnect.call_args.args[1], oracle.INNER_CONNECT_DIRECT_PORT_A
        )
        self.assertEqual(
            innerconnect.call_args.args[2], oracle.INNER_CONNECT_DIRECT_PORT_B
        )
        # scikit-rf's innerconnect itself performs one internal conversion to
        # pseudo; the second call is the fixture's explicit public restoration.
        explicit_restorations = [
            call
            for call in renormalize.call_args_list
            if call.kwargs == {"s_def": "power"}
        ]
        self.assertEqual(len(explicit_restorations), 1)
        self.assertEqual(explicit_restorations[0].args[1].shape, (3, 3))
        self.assertEqual(
            document["metadata"]["wave_definitions"]["inner_connect_raw"],
            "pseudo",
        )
        self.assertEqual(document["metadata"]["wave_definitions"]["output"], "power")

    def test_checker_rejects_raw_pseudo_omission_and_every_contract_field(self) -> None:
        # Reproduce the raw public output to prove that selecting it instead of
        # the explicitly restored S is not hidden by the 1e-12 output policy.
        data = self.fixture["data"]
        source_s = self.np.asarray(
            self._complex_array(data["s"]), dtype=self.np.complex128
        )
        source_z0 = self.np.asarray(
            self._complex_array(data["z0_ohm"]), dtype=self.np.complex128
        )
        frequency = self.np.asarray(data["frequency_hz"], dtype=self.np.float64)
        raw = self.skrf.network.innerconnect(
            self.skrf.Network(
                f=frequency,
                s=source_s,
                z0=source_z0,
            ...
            oracle.INNER_CONNECT_DIRECT_PORT_A,
            oracle.INNER_CONNECT_DIRECT_PORT_B,
        )
        raw_document = copy.deepcopy(self.fixture)
        raw_document["data"]["s_inner_connected"] = oracle._complex_array(raw.s)
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / self.case.path.name
            path.write_bytes(oracle._canonical_bytes(raw_document))
            self.assertEqual(
                oracle._check_numeric_fixture(
                    path, self.fixture, "s_inner_connected"
                ),
                1,
            )
        ...
        for metadata_path in (
            ("wave_definitions", "inner_connect_raw"),
            ("wave_definitions", "output"),
            ("junction_ports", "a"),
            ("port_order", "output"),
            ("reference_impedance", "selected_references_unequal"),
        ):
            with self.subTest(metadata_path=metadata_path):
                drifted = copy.deepcopy(self.fixture)
                parent, key = metadata_path
                if parent == "port_order":
                    drifted["metadata"][parent][key][0] = 4
                elif parent == "junction_ports":
                    drifted["metadata"][parent][key] = 0
                elif parent == "reference_impedance":
                    drifted["metadata"][parent][key] = False
                else:
                    drifted["metadata"][parent][key] = (
                        "power" if key == "inner_connect_raw" else "pseudo"
                    )
                with tempfile.TemporaryDirectory() as directory:
                    path = Path(directory) / self.case.path.name
                    path.write_bytes(oracle._canonical_bytes(drifted))
...
class DirectYToSRegistrationAndCheckerTests(unittest.TestCase):
    """Protect the singular-capable direct Y→S oracle contract."""
    ...
    def test_rank_deficient_case_is_registered_and_reconstructed_independently(self) -> None:
        case_id = next(iter(DIRECT_Y_TO_S_CASE_SPECS))
        spec = DIRECT_Y_TO_S_CASE_SPECS[case_id]
        registered = {case.case_id: case for case in oracle._CASES}
        case = registered[case_id]
        self.assertEqual(case.path, oracle.DIRECT_Y_TO_S_SINGULAR_FIXTURE)
        self.assertEqual(case.path.stem, case_id)
        self.assertEqual(case.comparison, "numeric_output")
        self.assertEqual(case.numeric_output_key, "s")
        ...
        self.assertEqual(
            ...
            "A=F(I+GY), B=F(I-conj(G)Y), solve S A=B",
        )
        self.assertEqual(metadata["random_seed"], spec["seed"])
        self.assertEqual(metadata["input_unit"], spec["input_unit"])
        self.assertEqual(metadata["output_unit"], spec["output_unit"])
        self.assertEqual(metadata["input_rank"], [2, 2, 2])
        self.assertEqual(
            metadata["reference_impedance"],
            {
                "complex": True,
        ...
        frequency, direct_y, direct_z0 = oracle._direct_y_to_s_singular_inputs(self.np)
        self.assertEqual(data["frequency_hz"], [float(value) for value in frequency])
        self._nested_complex_equal(data["y_s"], direct_y.tolist())
        self._nested_complex_equal(data["z0_ohm"], direct_z0.tolist())
        self.assertEqual(metadata["tolerance_policy"]["rtol"], 1e-12)
        self.assertEqual(metadata["tolerance_policy"]["atol"], 1e-12)
        self.assertIn("rank-deficient", metadata["tolerance_policy"]["justification"])
        self.assertTrue(
            all(
                math.isfinite(self._complex(value).real)
                and math.isfinite(self._complex(value).imag)
    ...
    def test_direct_builder_uses_explicit_power_definition(self) -> None:
        case_id = next(iter(DIRECT_Y_TO_S_CASE_SPECS))
    ...
    def test_checker_tolerates_only_direct_output(self) -> None:
        case_id = next(iter(DIRECT_Y_TO_S_CASE_SPECS))
        case = {case.case_id: case for case in oracle._CASES}[case_id]
        fixture = oracle._read_canonical_json(case.path)
        adjusted = json.loads(json.dumps(fixture))
        adjusted["data"]["s"][0][0][0]["real"] += 1e-13
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / case.path.name
            path.write_bytes(oracle._canonical_bytes(adjusted))
        ...
        drifted["data"]["y_s"][0][0][0]["real"] += 1e-3
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / case.path.name
            path.write_bytes(oracle._canonical_bytes(drifted))
            self.assertEqual(
...
class MixedOpenTerminationRegistrationAndCheckerTests(unittest.TestCase):
    """Protect the explicit-open/mixed-load termination oracle contract."""

    @classmethod
    def setUpClass(cls) -> None:
        cls.np, cls.skrf = oracle._load_dependencies()
        cls.case_id = oracle.TERMINATE_MIXED_OPEN_CASE_ID
        cls.case = {case.case_id: case for case in oracle._CASES}[cls.case_id]
        cls.fixture = oracle._read_canonical_json(cls.case.path)
    ...
    def test_case_is_registered_with_explicit_open_contract(self) -> None:
        self.assertEqual(self.case.path, oracle.TERMINATE_MIXED_OPEN_FIXTURE)
        self.assertEqual(self.case.path.stem, self.case_id)
        self.assertEqual(self.case.comparison, "numeric_output")
        self.assertEqual(self.case.numeric_output_key, "s_terminated")
        ...
        data = self.fixture["data"]
        self.assertEqual(metadata["case_id"], self.case_id)
        self.assertEqual(metadata["operation"], "terminate_port_power")
        self.assertEqual(metadata["numpy_version"], oracle.EXPECTED_NUMPY_VERSION)
        self.assertEqual(metadata["scikit_rf_version"], oracle.EXPECTED_SCIKIT_RF_VERSION)
        self.assertEqual(metadata["scikit_rf_commit"], oracle.EXPECTED_SCIKIT_RF_COMMIT)
        self.assertEqual(metadata["random_seed"], 20_260_963)
        self.assertEqual(metadata["wave_definition"], "power")
        self.assertEqual(metadata["port"], 2)
        self.assertEqual(metadata["port_order"]["input"], [0, 1, 2, 3, 4])
        self.assertEqual(metadata["port_order"]["output"], [0, 1, 3, 4])
        ...
        self.assertEqual(
            ...
            [
                ...
                {
                    "kind": "impedance_ohm",
                    "value": {"imag": 7.0, "real": 31.0},
                },
                {"kind": "open"},
                {
                    "kind": "impedance_ohm",
                    "value": {"imag": 4.0, "real": -17.0},
                },
            ],
        )
        self.assertEqual(metadata["wave_definitions"]["input"], "power")
        self.assertEqual(metadata["wave_definitions"]["load"], "power")
        self.assertEqual(metadata["wave_definitions"]["output"], "power")
        self.assertIn("renormalize", metadata["wave_definitions"]["restoration"])
        self.assertIn("Open", metadata["tolerance_policy"]["justification"])
        self.assertIn("only data.s_terminated", metadata["tolerance_policy"]["comparison"])

    def test_public_builder_uses_z2s_for_finite_loads_and_restores_power_output(self) -> None:
        with mock.patch.object(
            self.skrf.network,
            "z2s",
            wraps=self.skrf.network.z2s,
        ...
        for call in conversion.call_args_list:
            args, kwargs = call
            self.assertEqual(len(args), 1)
            self.assertEqual(args[0].shape, (1, 1, 1))
            self.assertEqual(kwargs["z0"].shape, (1, 1))
            self.assertEqual(kwargs["s_def"], "power")
        ...
        self.assertEqual(connect_kwargs, {})
        self.assertEqual(connect_args[1], 2)
        self.assertIsInstance(connect_args[0], self.skrf.Network)
        self.assertIsInstance(connect_args[2], self.skrf.Network)
        self.assertEqual(connect_args[3], 0)
        self.assertEqual(connect_args[0].s_def, "power")
        self.assertEqual(connect_args[2].s_def, "power")
        load_network = connect_args[2]
        self.np.testing.assert_array_equal(
            load_network.s[[0, 2], 0, 0],
            self.np.ones(2, dtype=self.np.complex128),
        )

        raw_expected = self.skrf.network.connect(*connect_args)
        restored_expected = raw_expected.copy()
        restored_expected.renormalize(raw_expected.z0, s_def="power")
        self.assertEqual(restored_expected.s_def, "power")
        actual = self.np.asarray(
            self._complex_array(document["data"]["s_terminated"]),
            dtype=self.np.complex128,
        )
        ...
        self.np.testing.assert_array_equal(
            self.np.asarray(
                self._complex_array(document["data"]["z0_survivor_ohm"])
            ),
            restored_expected.z0,
        )
        self.assertEqual(document["metadata"]["wave_definitions"]["output"], "power")

    def test_checker_tolerates_only_mixed_terminated_s_output(self) -> None:
        adjusted = copy.deepcopy(self.fixture)
        adjusted["data"]["s_terminated"][0][0][0]["real"] += 1e-13
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / self.case.path.name
        ...
        for field in ("s", "z0_ohm", "loads", "z0_survivor_ohm", "metadata"):
            with self.subTest(field=field):
                drifted = copy.deepcopy(self.fixture)
                if field == "s":
                    drifted["data"]["s"][0][0][0]["real"] += 1e-3
