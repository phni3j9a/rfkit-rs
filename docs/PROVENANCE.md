...
| `crates/rfkit-core/src/termination.rs`; `crates/rfkit-core/src/lib.rs`; `crates/rfkit-core/tests/public_termination.rs` | Kurokawa power-wave equations plus physical boundary `V_k=-Z_L I_k`; scikit-rf behavior reference | Kurokawa DOI `10.1109/TMTT.1965.1125964`; scikit-rf commit `bd651e923cac6020de49a096e1d7e9b5f949f884` (`v2.0.1`) | Issue #114 generalizes the historical finite physical-load operation to one explicit `PortLoad` boundary API | REWRITE | BSD-3-Clause for scikit-rf behavior reference; published mathematics | The finite branch REUSES the existing local checked evaluator and preserves its successful domain, arithmetic order, `d==0` handling, diagnostics, and source/survivor semantics. The `PortLoad::Open` branch is an independent REWRITE of the physical `I_k=0` boundary, yielding `1/(1-S_kk)` directly; it never represents Open as infinity/NaN or a large impedance and does not copy third-party code. The retired #86 name has no alias in unpublished 0.x. |
...
| `tools/oracle/generate_oracle.py`; `tools/oracle/fixtures/power_wave_terminate_port_impedance_five_port_complex_z0.json`; `crates/rfkit-core/tests/oracle_termination.rs` | scikit-rf public `z2s`/`Network`/`connect` behavior | `bd651e923cac6020de49a096e1d7e9b5f949f884` (`v2.0.1`); NumPy `2.5.1`; seed `20260950` | Historical finite fixture, now exercised through `terminate_port_power` with `PortLoad::ImpedanceOhm`: asymmetric five-port source, selected port 2, explicit `[0, 38+12j, 73-9j]` ohm loads, and original-order survivors `[0,1,3,4]` | REWRITE | BSD-3-Clause | Inputs and metadata are independently authored and generated through public scikit-rf calls; only `s_terminated` is numeric-tolerance output (`rtol=1e-12`, `atol=1e-12`). No scikit-rf source, output values, or fixture values were copied; Rust compares only expected S while deriving survivor z0 independently. |
| `tools/oracle/generate_oracle.py`; `tools/oracle/fixtures/power_wave_terminate_port_mixed_open_five_port_complex_z0.json`; `crates/rfkit-core/tests/oracle_termination.rs`; `crates/rfkit-touchstone/tests/public_termination_workflow.rs`; `crates/rfkit-touchstone/examples/terminate_port_touchstone.rs` | Kurokawa physical open/finite-load boundaries; scikit-rf public `z2s`/`Network`/`connect` behavior; IBIS Open Forum Touchstone specification | Kurokawa DOI `10.1109/TMTT.1965.1125964`; scikit-rf commit `bd651e923cac6020de49a096e1d7e9b5f949f884` (`v2.0.1`); NumPy `2.5.1`; Touchstone Format Specification v2.1 (2024-01-26); seed `20260963` | Independent mixed physical-load oracle/workflow evidence: four-frequency asymmetric five-port source, selected port 2, unequal complex frequency-dependent positive-real z0, tagged `[Open, 31+7j, Open, -17+4j]` ohm profile, explicit power-wave restoration/check, and original-order survivors `[0,1,3,4]`; local Touchstone profile `[Open, 38+12j, 0]` additionally exercises exact open, finite, and short boundaries before unchanged writer/read | REWRITE | Published mathematics; scikit-rf BSD-3-Clause; IBIS Open Forum specification | The fixture input and tagged load schema are independently authored; expected S is obtained only through public scikit-rf load construction and one public `connect` call, with the result explicitly restored and checked as power waves. Frequency, source S/z0, load tags, survivor z0, order, wave metadata, and operation are exact fields; only reduced S uses strict `rtol=1e-12`, `atol=1e-12`. Rust and Touchstone physical-boundary checks are independent equations/workflow evidence; no scikit-rf source, expected output, instrument material, or fixture value is copied. |
...
| `crates/rfkit-touchstone/examples/terminate_port_touchstone.rs`; `crates/rfkit-touchstone/tests/public_termination_workflow.rs` | Kurokawa physical boundary equations; Touchstone v1.0 specification | Kurokawa DOI `10.1109/TMTT.1965.1125964`; IBIS Open Forum Touchstone Format Specification v2.1 (2024-01-26) | Focused ingress → mixed Open/finite/short termination → independent reduction and physical-boundary checks → writer/read with common 50 Ω survivors | REWRITE | Published mathematics; IBIS Open Forum specification | Literal asymmetric five-port text, tagged `PortLoad` profile, analytical reduction, arbitrary-excitation V/I checks, and source/load immutability checks are independently authored. The existing writer is used only under its declared common finite positive-real reference contract; no hidden writer renormalization, open sentinel, mixed-mode extension, source copying, or broader Touchstone promise is introduced. |
...
Issue #102 added the historical Full-only oracle input. Issue #112 generalizes
that boundary to one `parse_touchstone_v2_0_s` reader and adds independent
Lower and Upper N=3 inputs; every expected value comes from the same pinned
public parser and not from the Rust parser or a v1 round trip. The pinned
two-port triangular `21_12` transpose-before-mirror defect (including the
required Lower/`21_12` example and analogous Upper path) is documented as a
reference discrepancy and is not serialized as an expected value.
...
| `crates/rfkit-touchstone/src/v2.rs`; `crates/rfkit-touchstone/tests/public_v2_parser.rs`; `crates/rfkit-touchstone/tests/public_v2_workflow.rs`; `crates/rfkit-touchstone/examples/touchstone_v2_full_workflow.rs` | IBIS Open Forum Touchstone Format Specification; scikit-rf behavior reference | Touchstone 2.0 Specification (ratified 2009-04-24, pp. 4–13, 25); scikit-rf `bd651e923cac6020de49a096e1d7e9b5f949f884` (`v2.0.1`) | Issue #102 historical Full-only parser, superseded by Issue #112's sole Full/Lower/Upper reader with per-port real references and v1 writer workflow | REWRITE | IBIS Open Forum specification; scikit-rf BSD-3-Clause | Dedicated Rust state machine independently authored from the v2 contract; Full preserves arbitrary matrices and Lower/Upper expand by plain transpose including the diagonal. v1 remains untouched. Mixed-mode, noise, information, non-S, complex references, unknown directives, malformed counts, semantic vendor comments, and trailing content remain explicit rejections. No scikit-rf source, standard example, or fixture value was copied. |
| `tools/oracle/generate_oracle.py`; `tools/oracle/fixtures/touchstone_v2_0_s_full_three_port.json`; `crates/rfkit-touchstone/tests/oracle_touchstone_v2.rs` | scikit-rf public `Touchstone` parser; NumPy | scikit-rf `bd651e923cac6020de49a096e1d7e9b5f949f884` (`v2.0.1`); NumPy `2.5.1`; no random seed | Canonical public-parser Touchstone 2.0 Full-matrix three-port input with continued `[Reference]` and unequal `[37,61,83]` ohm references | REWRITE | BSD-3-Clause | Source text and metadata are independently authored. Rust calls only `parse_touchstone_v2_0_s`; frequency, references, text, shape, and metadata are exact, while only S uses strict `rtol=1e-12`, `atol=1e-12` tolerance. Existing canonical numeric values are unchanged. |
| `tools/oracle/generate_oracle.py`; `tools/oracle/fixtures/touchstone_v2_0_s_lower_three_port.json`; `crates/rfkit-touchstone/tests/oracle_touchstone_v2.rs` | IBIS Open Forum Touchstone 2.0 Specification; scikit-rf public `Touchstone` parser; NumPy | Touchstone 2.0 Specification (ratified 2009-04-24, pp. 11, 13–18); scikit-rf `bd651e923cac6020de49a096e1d7e9b5f949f884` (`v2.0.1`); NumPy `2.5.1`; no random seed | Issue #112 Lower-matrix public-parser three-port fixture with compact six-pair records, continued `[Reference]`, unequal real `[25,50,75]` references, and nonzero imaginary entries | REWRITE | IBIS Open Forum specification; BSD-3-Clause | Literal source text, metadata, references, dimensions, and frequencies are independently authored and exact. Expected S alone comes from pinned public `Touchstone`; the checker uses only `rtol=1e-12`, `atol=1e-12`. Plain-transpose expansion is tested; no scikit-rf code, standard example, or fixture value was copied. |
| `tools/oracle/generate_oracle.py`; `tools/oracle/fixtures/touchstone_v2_0_s_upper_three_port.json`; `crates/rfkit-touchstone/tests/oracle_touchstone_v2.rs` | IBIS Open Forum Touchstone 2.0 Specification; scikit-rf public `Touchstone` parser; NumPy | Touchstone 2.0 Specification (ratified 2009-04-24, pp. 11, 13–18); scikit-rf `bd651e923cac6020de49a096e1d7e9b5f949f884` (`v2.0.1`); NumPy `2.5.1`; no random seed | Issue #112 independent Upper-matrix public-parser three-port fixture with compact six-pair records, continued `[Reference]`, unequal real `[25,50,75]` references, and nonzero imaginary entries | REWRITE | IBIS Open Forum specification; BSD-3-Clause | The Upper text is independently authored rather than derived from Lower or Full. Text, metadata, references, dimensions, and frequencies are exact; only decoded S uses strict `rtol=1e-12`, `atol=1e-12`. The fixture records pinned behavior for N=3 only and does not canonicalize the pinned two-port `21_12` uninitialized-triangle defect. |
