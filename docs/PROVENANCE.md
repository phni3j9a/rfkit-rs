...
NumPy `2.5.1`, random seed `20260955`, determinant evidence, and the separate
`P solve(S,I) P` check are recorded in fixture metadata. The canonical
fixture's unequal real-positive references and output-only
...
|---|---|---|---|---|---|---|
| `crates/rfkit-core/src/inverse_cascade.rs`; `crates/rfkit-core/src/lib.rs`; `crates/rfkit-core/tests/public_inverse_cascade.rs`; `crates/rfkit-core/tests/oracle_inverse_cascade.rs` | Kurokawa power-wave reversal equations; scikit-rf public behavior reference | Kurokawa DOI `10.1109/TMTT.1965.1125964`; scikit-rf `bd651e923cac6020de49a096e1d7e9b5f949f884` (`v2.0.1`) | Issue #94 fixed-group inverse cascade `S_inverse = P S^-1 P` | REWRITE | Published mathematics; scikit-rf BSD-3-Clause | The checked Rust kernel solves full S and both directional transmission blocks with the existing exact-pivot multiple-RHS solver; no dense inverse, scikit-rf source, or fixture values are copied. Public diagnostics distinguish full-S/forward/reverse stages. Local coverage includes complex/signed references, malformed and arithmetic failures, ideal/coupled/larger even-port invariants, and both-order physical cancellation. |
| `tools/oracle/generate_oracle.py`; `tools/oracle/fixtures/power_wave_inverse_cascade_four_port_real_unequal_z0.json` | scikit-rf; NumPy | scikit-rf `bd651e923cac6020de49a096e1d7e9b5f949f884` (`v2.0.1`); NumPy `2.5.1`; random seed `20260955` | Public `Network.inv` expected S plus independent NumPy `P solve(S,I) P` check | REWRITE | BSD-3-Clause | Independently generated three-frequency four-port real-positive unequal-reference fixture with exact group/shape/frequency/input/reference metadata, determinant evidence, and output-only `rtol=1e-12`/`atol=1e-12`; source S/z0 and conjugated swapped output references are exact contract fields. |
| `crates/rfkit-touchstone/tests/public_inverse_cascade_workflow.rs`; `crates/rfkit-touchstone/examples/inverse_cascade_touchstone.rs` | Kurokawa physical V/I equations; IBIS Open Forum Touchstone specification | Kurokawa DOI `10.1109/TMTT.1965.1125964`; Touchstone Format Specification v2.1 (2024-01-26) | Two-port Touchstone ingress → direct cascade → left-first/right-first inverse removal → explicit common-reference renormalization → write/read | REWRITE | Published mathematics; IBIS Open Forum specification | Independent local LEFT/DUT/RIGHT sweeps and port bookkeeping use existing public direct connection, inverse, renormalization, and writer APIs. No hidden orientation, automatic calibration, copied code, or broad file-format promise is introduced. |
