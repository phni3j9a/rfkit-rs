...
| `tools/oracle/generate_oracle.py`; `tools/oracle/fixtures/max_singular_value_power_four_port_complex_z0.json`; `tools/oracle/test_generate_oracle.py` | NumPy scalar SVD; scikit-rf public `Network.is_passive` | NumPy `2.5.1`; scikit-rf `2.0.1` at `bd651e923cac6020de49a096e1d7e9b5f949f884`; random seed `20260957` | Canonical four-port multi-frequency scalar SVD outputs plus limited one-sample Boolean passivity evidence | REWRITE | NumPy BSD-3-Clause; scikit-rf BSD-3-Clause | Inputs are independently generated and normalized to target maxima `[0.35, 0.65, 1.35, 1.85]`; only `data.sigma_max` is output-tolerance compared at `rtol=1e-12`, `atol=1e-12`, while input/reference/metadata/`data.is_passive` fields are exact. No source, expected output, or fixture values are copied. |
